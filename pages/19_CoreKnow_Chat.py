...
<style>
    ...
    * { font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; }
    ...
    /* NEW — CoreKnow brand name under brain logo */
    .brand-name {
        font-size: 2.8rem;
        font-weight: 900;
        letter-spacing: -1px;
        background: linear-gradient(135deg, #00e5ff 0%, #7c4dff 50%, #00c853 100%);
    ...
    @keyframes brandGlow {
        from { filter: drop-shadow(0 0 10px rgba(0, 229, 255, 0.4)); }
        to { filter: drop-shadow(0 0 25px rgba(124, 77, 255, 0.8)); }
    }
    .brand-tagline {
        font-size: 0.75rem;
        color: #6b6b80;
    ...
    .suggestion-title { color: #ececec; font-weight: 500; font-size: 0.9rem; }
    .suggestion-sub { color: #8e8ea0; font-size: 0.75rem; margin-top: 0.2rem; }

    /* ===== CHAT MESSAGES ===== */
    .chat-row-user { display: flex; justify-content: flex-end; padding: 1rem 0; gap: 1rem; }
    .chat-row-ai { display: flex; justify-content: flex-start; padding: 1rem 0; gap: 1rem; }
    .chat-avatar-user {
        width: 32px; height: 32px; border-radius: 50%;
        background: linear-gradient(135deg, #7c4dff, #00e5ff);
        display: flex; align-items: center; justify-content: center;
        font-weight: 700; font-size: 0.8rem; color: #0f0f0f; flex-shrink: 0;
    }
    .chat-avatar-ai {
        width: 32px; height: 32px; border-radius: 50%;
    ...
    .bubble-user {
        background: #2f2f2f; border-radius: 16px; padding: 0.85rem 1.1rem;
        max-width: 75%; color: #ececec; font-size: 0.95rem; line-height: 1.6;
        word-wrap: break-word;
    }
    .bubble-ai {
        background: transparent; padding: 0.85rem 0; max-width: 85%;
        color: #ececec; font-size: 0.95rem; line-height: 1.7;
        word-wrap: break-word;
    }
    .bubble-ai p { margin: 0.5rem 0; }
    .bubble-ai code {
        background: #1a1a1a; padding: 2px 6px; border-radius: 4px;
        font-family: 'Courier New', monospace; font-size: 0.85rem; color: #00e5ff;
    }
    .bubble-ai pre {
        background: #1a1a1a; padding: 1rem; border-radius: 8px;
        overflow-x: auto; border: 1px solid #2a2a2a;
    }
    .bubble-ai pre code { background: transparent; padding: 0; color: #00e5ff; }

    .timestamp { font-size: 0.7rem; color: #6b6b80; margin-top: 0.3rem; }

    .input-container {
        position: fixed; bottom: 0; left: 0; right: 0;
        background: linear-gradient(to top, #0f0f0f 60%, transparent);
        padding: 1.5rem 1rem 2rem 1rem; z-index: 999;
    }
...
with st.sidebar:
    ...
    <div style="text-align:center; padding: 1rem 0 1.5rem 0;">
        ...
        <div style="font-family: 'Inter'; font-weight: 900; font-size: 1.4rem;
...
if not st.session_state.messages:
    # WELCOME SCREEN WITH COR EKNOW BRANDING
    st.markdown("""
    <div class="welcome-container">
        <div class="brain-logo">🧠</div>
        <div class="brand-name">CoreKnow</div>
        <div class="brand-tagline">AI Knowledge Engine</div>
...
with col_input:
    user_input = st.text_area(
        ...
        placeholder="Ask CoreKnow anything...",
        label_visibility="collapsed",
        height=56,
        key="chat_input"
    )
...
def do_send(question, image_text=""):
    ...
    placeholder.markdown(
        '<div class="chat-row-ai">'
        '<div class="chat-avatar-ai">🧠</div>'
        '<div class="bubble-ai">🧠 CoreKnow is thinking...</div>'
        '</div>',
        unsafe_allow_html=True
    )
