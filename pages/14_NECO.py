...
st.set_page_config(page_title="NECO", page_icon="📚", layout="wide")
apply_theme()
...
st.markdown('<div class="ck-title">📚 ' + NECO + '</div>', unsafe_allow_html=True)
...
try:
    subjects_res = supabase.table("education_syllabi").select("subject").eq("exam", NECO).execute()
    subjects = sorted(list(set([s["subject"] for s in subjects_res.data])))
except:
    subjects = []

if not subjects:
    st.warning("📭 No subjects yet for " + NECO + ".")
...
if subject:
    ...
    try:
        topics_res = supabase.table("education_syllabi") \
            ...
            .eq("exam", NECO).eq("subject", subject).order("topic_number").execute()
