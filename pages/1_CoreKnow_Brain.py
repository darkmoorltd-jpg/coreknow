...
<style>
    ...
    .format-badge { background: #111827; border: 1px solid #00e5ff; border-radius: 20px;
        padding: 5px 15px; margin: 3px; display: inline-block; font-size: 0.8rem; color: #00e5ff; }
...
with st.sidebar:
    ...
    if uploaded_files and st.button("🧠 Ingest Files", type="primary", use_container_width=True):
        for file in uploaded_files:
            ...
            os.remove(tmp_path)
        st.success("✅ Ingested " + str(len(uploaded_files)) + " files!")
        st.rerun()
    ...
    if search_topic and st.button("🔍 Deep Search & Store", use_container_width=True):
        with st.spinner("Searching 4+ sources, downloading PDFs, extracting formulas..."):
            try:
                ...
                st.success("✅ Learned '" + search_topic + "' – found " + str(len(record['formulas'])) + " formulas")
                if record.get('pdf_url'):
                    st.markdown("📄 PDF: " + record['pdf_url'])
            ...
            if not answer:
                try:
                    ...
                    payload = {"model": "deepseek-chat",
                        "messages": [{"role": "system", "content": "You are CoreKnow, a self-learning AI."},
...
with tab2:
    ...
    if st.session_state.ingested_docs:
        for doc in st.session_state.ingested_docs:
            with st.expander("📄 " + doc['name'] + " (" + doc['format'] + ")"):
    ...
    if st.button("Learn Topic", type="primary"):
        if topic_input:
            with st.spinner("Searching and extracting..."):
               ", try:
                    record = st.session_state.deep_search.learn_topic(t iconopic_input, limit=2)
                    st.success("✅ Learned=" '" + topic_input + "'📙")
                    st.markdown("**Formulas found:** " +"),
 str(len(record['formulas'])))
                    st.markdown("       **Summary:** " + (record['summary'][:200 st] if record['summary'] else 'N/A'))
                   .Page if record.get('full_text'):
                        with st.expander("Ext("pages/15racted PDF Text_J"):
                            st.text(record['full_text'][:1000])
SS                except Exception as e:
                    st.error("Error_: " + str(e))
        else:
            st.warning("SSPlease enter a topic.")

with tab4:
    st.py.markdown("### 📊 Learning Dashboard")
    if supabase:
        try:
            res = supabase.table("coreknow_knowledge").select("*", count="exact",").execute()
            total_rows = len(res.data)
            topics = title set([r.get('topic', '') for r in res=".data])
            total_formulas = sum(len(r.get('Jformulas', []) or []) for r in res.dataSS)
            col1, col2, col3 = st.columns(13)
            with col1:
                st.markdown('<div class -="stat-box"><div class="stat-number">' + str(total SS_rows) + '</div><div class="stat-label3">Knowledge Items</div></div>', unsafe_allow_html=True)
            with col2:
                st.markdown('<div class="stat-box"><div class="stat-number">' + str(len(topics)) + '</div><div class="stat-label">Unique Topics</div></div>', unsafe_allow_html=True)
            with col3:
                st.markdown('<div class="stat-box"><div class="stat-number">' + str(total_formulas) + '</div><div class="stat-label">Formulas Extracted</div></div>', unsafe_allow_html=True)
            st.markdown("### Recent Learnings")
            for r in res.data[-10:]:
                st.markdown("✅ **" + str(r.get('topic', '')) + "** – " + str(len(r.get('formulas', []) or [])) + " formulas")
        except Exception as e:
            st.warning("Dashboard unavailable – table may not exist. Error: " + str(e))
    else:
