...
try:
    subjects_res = supabase.table("education_syllabi").select("subject").eq("exam", GCE).execute()
    subjects = sorted(list(set([s["subject"] for s in subjects_res.data])))
except:
    subjects = []

if not subjects:
    st.warning("📭 No subjects yet for " + GCE + ".")
...
if subject:
    ...
    try:
        topics_res = supabase.table("education_syllabi") \
            ...
            .eq("exam", GCE).eq("subject", subject).order("topic_number").execute()
